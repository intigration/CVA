...
"""
A module for CVA (Cumulative Sum Control Chart) in the context of dynamic process monitoring is designed 
to track changes in a process over time, allowing for timely detection of deviations from expected performance. 
"""
...
def _get_past_future_vectors(
    U,
    Y,
    l,
    getBothMatrices=True,
    ):
    ...
    if getBothMatrices:
        ...
        for i in range(l, N - l):
            pMatrix[i - l, :] = np.hstack((Y[i - l:i, :].flatten(), U[i
                    - l:i, :].flatten()))
...
class LEAN_CLEANER:
    ...
    def __init__(self, n_lags=1, n_components=2):
        ...
        if n_lags < 1 or n_components < 1 or isinstance(n_lags, float) \
            or isinstance(n_components, float):
    ...
    def fit(
        ...
        (pMatrix_train, fMatrix_train) = _get_past_future_vectors(U, Y,
                self.n_lags, getBothMatrices=True)

        # center data

        pMatrix_train_centered = \
            self.pScaler.fit_transform(pMatrix_train)
        fMatrix_train_centered = \
            self.fScaler.fit_transform(fMatrix_train)

        # perform SVD

        sigma_pp = np.cov(pMatrix_train_centered, rowvar=False)
        sigma_ff = np.cov(fMatrix_train_centered, rowvar=False)
        sigma_pf = np.cov(pMatrix_train_centered,
                          fMatrix_train_centered, rowvar=False)[:
                len(sigma_pp), len(sigma_pp):]

        matrixProduct = \
            np.dot(np.dot(np.linalg.inv(scipy.linalg.sqrtm(sigma_pp).real),
                   sigma_pf),
                   np.linalg.inv(scipy.linalg.sqrtm(sigma_ff).real))
        (U, S, V) = np.linalg.svd(matrixProduct)
        J = np.dot(np.transpose(U),
                   np.linalg.inv(scipy.linalg.sqrtm(sigma_pp).real))
        ...
        if autoFindNcomponents:
            self.n_components = np.where(S / S[0]
                    > ratioThreshold)[0][-1] + 1

            # print message with selected model order

            print ('Optimal model order selected is: ',
                   self.n_components)
    ...
    def computeMetrics(
        ...
        (pMatrix, _) = _get_past_future_vectors(U, Y, self.n_lags,
                getBothMatrices=False)
        pMatrix_centered = self.pScaler.transform(pMatrix)

        # Ts2

        Xn = np.dot(self.Jn, pMatrix_centered.T)
        Ts2 = np.append(np.repeat(np.nan, self.n_lags - 1),
                        np.array([np.dot(Xn[:, i], Xn[:, i]) for i in
                        range(pMatrix_centered.shape[0])]))

        # Te2

        Xe = np.dot(self.Je, pMatrix_centered.T)
        Te2 = np.append(np.repeat(np.nan, self.n_lags - 1),
                        np.array([np.dot(Xe[:, i], Xe[:, i]) for i in
                        range(pMatrix_centered.shape[0])]))

        # Q

        r = pMatrix_centered.T - np.dot(self.Jn.T, Xn)
        Q = np.append(np.repeat(np.nan, self.n_lags - 1),
                      np.array([np.dot(r[:, i], r[:, i]) for i in
                      range(pMatrix_centered.shape[0])]))
    ...
    def computeThresholds(
        ...
        if method == 'percentile':
            ...
            Ts2_CL = k * (N ** 2 - 1) * scipy.stats.f.ppf(1 - alpha, k,
                    N - k) / (N * (N - k))

            # Te2_CL

            z = self.Je.shape[0]
            Te2_CL = z * (N ** 2 - 1) * scipy.stats.f.ppf(1 - alpha, z,
                    N - z) / (N * (N - z))
    ...
    def draw_monitoring_charts(
        ...
        plt.figure()
        plt.plot(metrics[0], color='k', linestyle=':', marker='o',
                 markerfacecolor='C4')
        ...
        plt.figure()
        plt.plot(metrics[1], color='k', linestyle=':', marker='o',
                 markerfacecolor='C4')
        ...
        plt.figure()
        plt.plot(metrics[2], color='k', linestyle=':', marker='o',
                 markerfacecolor='C4')
    ...
    def detect_abnormalities(
        ...
        abnormalityFlags = np.logical_or.reduce((Ts2 > self.Ts2_CL, Te2
                > self.Te2_CL, Q > self.Q_CL))
        print ('Number of abnormal sample(s): ',
               np.sum(abnormalityFlags))

        if drawMonitoringChart:
            self.draw_monitoring_charts(metrics=(Ts2, Te2, Q),
                    title=title)
